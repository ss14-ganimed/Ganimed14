ent-RubberStampMagisrat = Печать магистрата
    .suffix = DO NOT MAP
    .desc = Выглядит угрожающее. От нее веет неоспоримой мощью и величием.
ent-ADTRubberStampRemake = печать ПЕРЕДЕЛАТЬ
    .desc = Увидеть её после того, как долго подготавливал документ - настоящее мучение.

ent-ADTRubberStampDV = штамп Dar-Vaxed
    .desc = Штамп для представителей компании Dar-Vaxed.
    .suffix = ДВ, DO NOT MAP

ent-ADTRubberStampPathologist = Печать патологоанатома
    .suffix = DO NOT MAP
    .desc = От неё веет печалью и смертью.

ent-RubberStampCCC = Печать консультанта Центком
    .suffix = DO NOT MAP
    .desc = Смотря на эту печать, вы ощущаете, что её владелец - Потанциально опасный бюрократ. Возможно физически. Вы не знаете, сколько проблем или пользы она вам может принести в будущем.
